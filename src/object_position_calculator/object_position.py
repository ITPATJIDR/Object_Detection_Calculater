    ...
    def _process_and_send_image(self, image_byte_array):
        ...
        for detected in detections:
            ...
            calculator = newCoordinateCalculator(
                ...
                gps_lon=self.lng, image_width=640, image_height=640
                , detected_x=x, detected_y=y,
            ...
            cv2.putText(image, f"({results['coordinates_f'][0]:.15f}, {results['coordinates_f'][1]:.15f})", text_position_1, cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            
            text_position = (top_left[0], bottom_right[1] + 20)  # Position the text below the bottom-right corner
            cv2.putText(image, f"Distance between G and F: {results['distance_gf']:.8f} meters", text_position, cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
